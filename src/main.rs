...
USAGE:
subdomain_ip_lister saved/file/from/gobuser delineator
example:
  subdomain_ip_lister ./gobusterdns.txt | 
  will output subdomain|ipaddress
  subdomain_ip_lister ./gobusterdns.txt \\\\t
  will output subdomain (tab) ip address
    ";
    if args.len() != 3{
        print!("{}", usage);
    }
        ...
        if delin == &"tab".to_string(){
            ...
            for (subdomain, ip) in subdomain_ips{
                let outline = format!("{}{}{}", subdomain, &delin, ip);
                println!("{}", outline);
            }
        }
    }
